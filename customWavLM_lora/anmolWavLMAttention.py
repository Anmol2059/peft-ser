...
class CustomWavLMAttention(nn.Module):
    ...
    def __init__(
        ...
        # anmol added
        # LORA layers
        self.lora_intermediate = lora.Linear(embed_dim, lora_dim, r=lora_rank)
    ...
    def torch_multi_head_self_attention(
        ...
        attn_output_lora, attn_weights_lora = F.multi_head_attention_forward(
            ...
            add_zero_attn,
            self.dropout,
            self.lora_output.weight,
            self.lora_output.bias,
            self.training,
            key_padding_mask,
            output_attentions,
            gated_position_bias,
            use_separate_proj_weight=True,
            q_proj_weight=self.lora_output.weight,
            k_proj_weight=self.lora_output.weight,
            v_proj_weight=self.lora_output.weight,
        )
